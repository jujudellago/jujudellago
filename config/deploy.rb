...
task :link_frozen_rails, :roles => [:app] do
  run <<-EOF
      ln -s #{shared_path}/galleries #{latest_release}/public/galleries  ;  ln -s #{shared_path}/ruby #{latest_release}/vendor/bundle/ruby  ; cp #{shared_path}/.htaccess #{latest_release}/public/ ; cp   #{shared_path}/config/* #{latest_release}/config/
  EOF
end
